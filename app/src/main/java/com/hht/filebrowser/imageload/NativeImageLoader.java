...
public class NativeImageLoader {  
    ...
    private NativeImageLoader(){
        ...
        Log.e("run_memory", "run time max maxmemory "+ maxMemory);
       //用最大内存的1/4来存储图片  
     ...
     * @return 
     */  
    public Bitmap loadNativeImage(final String path, final int vieWidth, final int viewHeight, final NativeImageCallBack mCallBack){  
      //先获取内存中的Bitmap
      ...
      //若该Bitmap不在内存缓存中，则启用线程去加载本地的图片，并将Bitmap加入到mMemoryCache中
        if(bitmap == null){  
            mImageThreadPool.execute(new Runnable() {  
                ...
                public void run() {
                    // 先获取图片的缩略图
                    Bitmap bitmap = decodeThumbBitmapForFile(path, vieWidth,
                            viewHeight);

                    if (bitmap != null) {
                        // 将图片加入到内存缓存
                        if (mMemoryCache.get(path) == null || mMemoryCache.get(path).isRecycled()) {
